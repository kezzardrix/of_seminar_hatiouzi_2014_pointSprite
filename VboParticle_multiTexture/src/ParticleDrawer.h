...
public:
    ...
    void setup(){
        ...
        for(int i = 0; i < NUM; i++){
            ...
            mNormals[i].set(ofRandom(2,50),(int)ofRandom(0,mTextures.size()));//yの値でどのテクスチャを使うかランダム
        }
    ...
    void draw(){
        ...
        mPointSprite.setUniformTexture("tex1", mTextures[0], 0); //テクスチャの番号。シェーダーで宣言した順番で
        mPointSprite.setUniformTexture("tex2", mTextures[1], 1);
